...
def _sample_logit_softmax(raw_scores_dict, K, T, eps=1e-8):
    """
    Option A — sample from softmax(raw_scores / T) via multinomial.

    Raw scores are QK^T/√d_k (Kim's natural form). Dividing by T_struct=√d_k
    recovers the Boltzmann distribution at the structural temperature.
    ...
    for layer, r in raw_scores_dict.items():
        ...
        # Re-apply causal mask in LOGIT space
        # Bug #4 reversal: -inf, not 0.0
        upper = np.triu(np.ones((S, S), dtype=bool), k=1)
        for h in range(n_kv):
            scores = r[h].copy()
            ...
            # Sample row-by-row
            for i in range(S):
                logits = torch.from_numpy(scores[i] / T).float()  # /T_struct
                # Softmax — use torch's stable implementation
...
def _sample_logit_gumbel(raw_scores_dict, K, T, eps=1e-8):
    ...
    over the Hamiltonian's basins. This kernel matches that behavior in
    software so we can compare to real silicon when it arrives.
    """
    ...
    for layer, r in raw_scores_dict.items():
        ...
        for h in range(n_kv):
            scores = r[h].copy() / T          # divide by T_struct
            scores[upper] = -1e9              # logit-space causal mask
...
def run_test(model, tokenizer, prompts, tokens_per_prompt,
    ...
    if capture_invariant_failures > 0:
        print(f"\n  {red(f'WARNING: capture invariant failed on '
              f'{capture_invariant_failures} steps')}")
    else:
        print(f"\n  {green(f'capture invariant: PASSED on all {len(all_records)//4} steps')}")
