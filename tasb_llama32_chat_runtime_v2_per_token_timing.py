...
----------------------------------
  ...
  /backend qubo       switch to QUBO p-bit substrate emulator (CPU Gibbs)
        ...
        for step in range(cfg.max_new_tokens):
            ...
            if cfg.vanilla_mode:
                ...
                for layer in layers:
                    ...
                    _thrml_elapsed = time.time() - _thrml_t0
            ...
            if cfg.backend == "thrml":
                _thrml_str = (f"{_thrml_elapsed:.3f}s"
                               if _thrml_elapsed is not None else "n/a")
                print(f"  [TIMING] step={step:3d}  seq_len={_seq_len:4d}  "
                      f"thrml={_thrml_str:>8}  total_step={_step_elapsed:6.3f}s")
