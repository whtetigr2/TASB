...
def test_t1_alpha_zero_identity(model, tok):
    ...
    injector = LlamaAttentionInjector(
        {cap.layer_idx: DispatchEntry(capture=cap, p_thermo=p_thermo, alpha=0.0)})
...
def test_t2_alpha_one_replaces_attention(model, tok):
    ...
    injector = LlamaAttentionInjector(
        {cap.layer_idx: DispatchEntry(capture=cap, p_thermo=p_thermo, alpha=1.0)})
...
def test_t3_non_target_layers_untouched(model, tok):
    ...
    injector = LlamaAttentionInjector(
        {cap_l18_for_inject.layer_idx: DispatchEntry(
...
def test_t5_context_manager_cleanup(model, tok):
    ...
    injector = LlamaAttentionInjector(
        {cap.layer_idx: DispatchEntry(capture=cap, p_thermo=p_thermo, alpha=0.5)})
...
def test_t7_alpha_monotonicity(model, tok):
    ...
    for alpha in alphas:
        injector = LlamaAttentionInjector(
            {cap.layer_idx: DispatchEntry(capture=cap, p_thermo=p_thermo, alpha=alpha)})
        with injector.inject():
            with torch.no_grad():
                out = model(**inputs, use_cache=False)
...
def test_t8_end_to_end_smoke(model, tok):
    ...
    injector = LlamaAttentionInjector(
        {cap.layer_idx: DispatchEntry(capture=cap, p_thermo=p_thermo, alpha=0.3)})
