...
TEST MAP (spec requirement -> test method):
  ...
  Req 13: test_13_multilayer_gumbel_backend_raises
...
class TestMultilayerRefactor(unittest.TestCase):
    ...
    def _get_seed_for_layer(self):
        from tasb_pipeline_v2 import seed_for_layer
    ...
    # ── Test 13: multi-layer gumbel backend raises ───────────────────────
    def test_13_multilayer_gumbel_backend_raises(self):
        """Req 13: layer_idx=[18,24], backend='gumbel' raises ValueError."""
        print("\n  Test 13: multi-layer gumbel backend raises ValueError")
        ids = self.input_ids_list[0]
        with self.assertRaises(ValueError) as ctx:
            self._run_list(
                ids, layers=[LAYER_IDX, LAYER_IDX2],
                alpha=ALPHA, backend='gumbel')
        msg = str(ctx.exception)
        self.assertIn('gumbel', msg)
        self.assertIn('exact', msg)
        print(f"    PASS: raised ValueError: {ctx.exception}")
